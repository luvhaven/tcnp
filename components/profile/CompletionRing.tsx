...
export function CompletionRing({
  ...
  const showInnerLabel = showLabel && !!label && size >= 80
  ...
  return (
    <div className="relative inline-flex items-center justify-center shrink-0" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center p-0.5 text-center pointer-events-none">
        <AnimatedPercent spring={spring} className={fontSizeClass} />
        {showInnerLabel && (
...
function AnimatedPercent({ spring, className }: { spring: ReturnType<typeof useSpring>; className?: string }) {
  const rounded = useTransform(spring, (v) => `${Math.round(v)}%`)
  return <motion.span className={`stat-figure leading-none ${className || "text-2xl font-bold"}`}>{rounded}</motion.span>
}
